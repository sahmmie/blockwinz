...
export class PlinkoService {
  ...
  constructor(
    ...
    private readonly plinkoRepository: PlinkoRepository,
  ) {}
  ...
  private calculateMultiplier(path: number[], rows: number, risk: string): number {
  ...
  async getPlinkoResult(
    ...
    return await this.db
      .transaction(async (tx) => {
        ...
        const gameRow = await this.plinkoRepository.insertPlinkoGame(txDb, {
          userId,
          seedId,
