...
export class MinesService {
  ...
  constructor(
    ...
    private readonly minesGameRepository: MinesRepository,
  ) {}
  ...
  async start(
    ...
    return await this.db
      .transaction(async (tx) => {
        ...
        const activeRow = await this.minesGameRepository.findOpenGameByUserId(
          txDb,
          String(player._id),
        );
        if (activeRow) {
          this.logger.log(
            `MINES START Player ID: ${player._id} Active game found with Game ID: ${activeRow.id}`,
          );
          return this.buildGameResponseFromRow(activeRow, []);
        }
        ...
        const gameRow = await this.minesGameRepository.insertGame(txDb, {
          userId,
          seedId,
    ...
    return await this.db
      .transaction(async (tx) => {
        ...
        if (hitMine) {
          ...
          await this.minesGameRepository.updateGameById(txDb, gameRow.id, {
            multiplier: '0',
  ...
  private async performCashout(
    ...
    await this.minesGameRepository.updateGameById(tx, gameRow.id, {
  ...
  async getActiveGame(
    ...
    const db = tx ?? this.db;
    const row = await this.minesGameRepository.findOpenGameByUserId(
      db,
      playerId,
    );
  ...
  async closeInactiveGames(): Promise<void> {
    ...
    const games =
      await this.minesGameRepository.findStaleOpenGames(fiveMinuteAgo);
    ...
    for (const game of games) {
      await this.db
        .transaction(async (tx) => {
          ...
          await this.minesGameRepository.updateGameById(txDb, game.id, {
            betResultStatus: MinesGameStatus.FINISHED,
          });
        })
        .catch((error) => {
  ...
  async closeMineGame(gameId: string): Promise<void> {
    return await this.db
      .transaction(async (tx) => {
        ...
        const gameRow = await this.minesGameRepository.findOpenGameById(
          txDb,
          gameId,
        );
