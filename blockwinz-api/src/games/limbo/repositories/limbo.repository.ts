...
export class LimboRepository {
  private readonly logger = new Logger('Limbo Service');
  ...
  async getLimboResult(
    ...
    return await this.db
      .transaction(async (tx) => {
        ...
        if (!gameRow) throw new Error('Failed to create limbo game');
  ...
  generateLimboResultNumber(request: FairLogicGenerateFloatsDto): number {
    const limboOutcome =
